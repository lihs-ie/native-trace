...
applications/
  ...
  python-analyzer/  Python (FastAPI) — 音響計測 analyzer（GOP/韻律/TTS。compose service: analyzer、port 8788）
  golden-speaker/   Python — お手本音色変換サービス（compose profile: golden 有効時のみ起動、port 8789）
  aai/              Python — 音響→調音逆推定サービス（compose profile: aai 有効時のみ起動、port 8790）
...
- 注意: ローカル GHC は 9.12.2 の環境がある（CI / `tested-with` は 9.10.3 が正 — ローカルのみで通る警告・挙動差に注意）

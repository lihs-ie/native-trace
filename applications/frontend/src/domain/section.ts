...
export const createSectionIdentifier = (value: string): SectionIdentifier | null =>
  createNonEmptyBrandedString<SectionIdentifier>(value);

export const createSectionVersion = (value: number): Result<SectionVersion, DomainError> => {
  if (!Number.isInteger(value) || value < 1)
    return err(validationFailed("version", "版番号は1以上の整数である必要があります"));
...
export const createSectionBodyText = (value: string): Result<SectionBodyText, DomainError> => {
  const trimmed = value.trim();
  if (trimmed.length === 0) return err(validationFailed("bodyText", "本文は空にできません"));
  if (trimmed.length > MAX_BODY_TEXT_LENGTH)
    return err(
      validationFailed("bodyText", `本文は${MAX_BODY_TEXT_LENGTH}文字以内である必要があります`),
    );
  // 制御文字禁止（改行・タブ以外）
  if (/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/.test(trimmed)) {
    return err(validationFailed("bodyText", "本文に制御文字を含めることはできません"));
  }
  // 英字割合チェック
  const englishChars = (trimmed.match(/[a-zA-Z]/g) ?? []).length;
  if (englishChars / trimmed.length < MIN_ENGLISH_CHAR_RATIO) {
    return err(validationFailed("bodyText", "本文には英字を十分に含める必要があります"));
  }

...
class _FixedG2P:
    ...
    def __init__(self, phonemes: list[str]) -> None:
        self._ipa = IpaSequence(phonemes=tuple(PhonemeLabel(p) for p in phonemes))
...
class TestDtwComputeLag:
    ...
    def test_speech_rate_ratio_computed_from_waveforms(self) -> None:
        ...
        assert result.speech_rate_ratio is not None, (
            "waveform があるとき speechRateRatio は None でない"
        )

...
ADR-018 D2: to_formant_burg を発話全体で 1 回呼び、境界中点でサンプリングする。
             max_number_of_formants=5 (定数・本数) /
             maximum_formant=Hz 天井 (呼び出し側から受け取る)。
...
M-APD-19: 単一中点サンプリングを多点 median サンプリングに置換。区間内側 0.3〜0.7 の 5 点で
           ...
           forced-alignment が区間を過大に伸ばし幾何中点が
           voicing offset を越える音素（例: /r/）でも
...
def extract_phoneme_acoustics(
    ...
    try:
        ...
        # max_number_of_formants=5 は本数（定数）;
        # maximum_formant は Hz 天井（呼び出し側から受け取る）
        # NOTE: これらの引数を取り違えると全フォルマントが無効になる
        # 致命的バグ（D2 regression risk）。
        ...
        for boundary in boundaries:
            ...
            if duration_ms < 40:
                ...
                # forced-alignment が区間を過大伸張し幾何中点が
                # voicing offset を越える場合（例: /r/）でも
                # 内側の有声フレームから formant を取得できる。
                # F1/F2/F3 を独立に NaN 除去して median を採る。
...
def _get_formant_median(
    ...
    valid_values = [
        ...
        if (v := _get_formant_value(formants, formant_number, t)) is not None
    ]
    if not valid_values:
        return None

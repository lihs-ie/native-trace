...
2026-06-19 partial supersede（ADR-024）: 本 ADR の Non-goal「新 UI コンポーネント / 画面: …新規 `.tsx` は追加しない」と、M-APD-16 受入条件のうち `.tsx` ゼロ assert（`git diff --name-only HEAD -- applications/frontend/src/components/ | grep "\.tsx$"` = 0 件）は ADR-024（音響診断ビジュアルレイヤと数値スカラー契約拡張）が supersede する。acousticEvidence の measurement-only / categorical-enum / scoreImpact 不変（D5/D7）は維持される。M-APD-16 本体の rule-based howJa 方向ラベル分岐 Must 自体は不変。
...
- (ADR-024 supersede): 本 ADR の UI Non-goal（spec:449「新規 `.tsx` は追加しない」再掲 spec:233）と M-APD-16 受入の `.tsx` ゼロ assert（spec:413-414）は ADR-024（音響診断ビジュアルレイヤと数値スカラー契約拡張）が supersede する。`AcousticDiagnosisCard.tsx` が `ArticulationCard` 配下に追加され、7 本の数値スカラーが `AcousticEvidence` 契約に加わる。acousticEvidence の measurement-only / categorical-enum / scoreImpact 不変（D5/D7）は本 ADR 管轄として維持される。M-APD-16 本体の rule-based howJa 方向ラベル分岐 Must 自体も不変。
...
- (S-APD-5a) production speakerSex は 'unknown' のみ: M/F 活性化は UI 収集動線 + FE request-mapper + Haskell AnalyzerMetadata 3 層のゲートが全て揃って初めて有効になる。現状 3 層とも未配線のため runtime で M/F 分岐には到達しない（unit テストのみ被覆）。活性化時期は product 判断による。
- (S-APD-5b) /r/ rhoticity の根本原因（forced-alignment の区間膨張、実信号 ~290ms → aligner 773ms → 幾何中点が voicing offset を越え Praat get_value_at_time が NaN を返す）は ADR-001 管轄。M-APD-19 の多点サンプリング（区間内側 0.3〜0.7 の 5 点 median）は観測性の局所緩和であり、alignment 精度自体には手を入れない。

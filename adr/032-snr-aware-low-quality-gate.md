...
## D4 補正-2（2026-06-20 多クリップ検証 — 固定 floor 不成立を確定、production 昇格保留）

PROVISIONAL だった floor 0.5 を **13 の実音声クリップ**（libritts/kokoro + hello_world、各 SNR ladder {clean/20/10/5/0}dB を `/v1/analyze` に実測）で検証した結果、**固定 floor 設計そのものが不成立**と確定した（`.agent-evidence/snr-floor-validation/report.md`）:

- floor 0.5 が valid/confident-misdecode を正しく分離できたのは **2/13 クリップのみ**。同一 true-SNR での estimatedSnrDb の clip 間オフセットが **clean で ~11dB（-3.70〜+7.17）** ばらつく。
- floor 0.5 で **clean 録音 6/13 が誤 low_quality**（良音声を弾く）、**5dB confident-misdecode 4/13 がすり抜け**。valid 域と bad 域の estimatedSnrDb が ~6dB オーバーラップし、**どんな固定 floor も分離不能**（最良の固定値 -4.25 でも 16/65 誤判定、0.5 は 23/65）。
- WADA が保つのは **clip 内順序性のみ**（12/13 が true-SNR に単調）。絶対ゼロ点が clip 間で移動する（`K_clean_prior` の単一分布アンカー問題）。GOP elbow も clip 依存（clean/20/10/5 に散る）で「10dB valid / 5dB 崩れ」は単一 fixture 由来。

**よって ADR-032 を現設計（固定 WADA-absolute floor）で Proposed から昇格させない。** floor 0.5 は self-eval ハーネスの `noise_monotonicity` 除外用 PROVISIONAL 定数としてのみ残し、production `audioQualityMinSnrDb` には出さない（M-SNR-7 の「production 昇格」は未達のまま）。前進は**新しい設計判断（フォローアップ ADR）**を要する: (1) **per-clip-relative drop gate**（各ユーザの clean baseline からの低下量。Δ閾値 ~1.5dB で 4/65 誤判定＝6 倍改善。ただし stateless worker に per-user clean 基準が要る → ADR-031 の自己録音キャリブ baseline をアンカーにする） / (2) `K_clean_prior` を実音声で再較正（clean stdev ~3.5dB は実音声固有で 11dB スプレッドは閉じない見込み） / (3) 別推定器（VAD-ratio / minimum-statistics noise-floor）の spike。これも self-eval 検証が「合成 self-eval は閉じたが実クリップで製品劣化」を捕捉した結果である。

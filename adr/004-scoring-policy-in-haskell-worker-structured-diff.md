...
The worker returns a **structured diff**, not finished messages: the existing wire `FindingDto` extended with `phenomenon` (a closed enum: `substitution`, `omission`, `insertion`, `weakForm`, `linking`, `flap`, `assimilation`, `reduction`) and `gop: number | null`, with `messageJa = null`. The frontend `ImprovementMessageGenerator` fills `messageJa` keyed on `phenomenon` + `expected` + `detected` before the domain `AssessmentFinding` is constructed; the domain invariant (`messageJa` required) is preserved because generation always runs first.

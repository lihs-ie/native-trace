...
OSS Workerの解析処理は2プロセスに分割する。**Haskell worker**はリクエスト検証、解析オーケストレーション、採点ポリシー（GOP閾値→severity→scoreImpact→集約ScoreSet。厳しめ判定 REQ-NF-036 [非機能要件](../01-requirements/requirements-specification.md#8-非機能要件) を含む）、finding骨格、response構築を担う。**Python解析サービス（python-analyzer）**は生計測のみを担い、espeak-ng g2pによるexpected IPA、強制アラインメント、wav2vec2 phoneme-CTCによる各音素のGOP、detected IPA、連結境界の無音長、schwa実現、話速を返す。Haskell workerはg2p/音響解析を持たず、python-analyzerへ同期HTTPで委譲する（[ADR-001](../../adr/001-gop-based-pronunciation-error-detection.md), [ADR-002](../../adr/002-espeak-ng-ipa-unification-g2p-in-python.md), [ADR-004](../../adr/004-scoring-policy-in-haskell-worker-structured-diff.md)）。expected/detected両側の音素はespeak IPAインベントリに統一する（[ADR-002](../../adr/002-espeak-ng-ipa-unification-g2p-in-python.md)）。workerはmessageJaを生成せず、`phenomenon`と`gop`を加えた構造化diff（`messageJa=null`）を返し、改善文はfrontendの`ImprovementMessageGenerator`が後段で付与する（[ADR-004](../../adr/004-scoring-policy-in-haskell-worker-structured-diff.md)）。

MVPはpronunciationスコア（全音素GOP）を縦に1本貫通させることを範囲とする。本文一致のaccuracy（読み飛ばし/追加/置換）に用いる音声認識（whisper）と、強勢・イントネーション等のprosodyはMVPから外し、後続の縦切りで導入する（[ADR-003](../../adr/003-defer-whisper-asr-until-accuracy-slice.md)）。accuracy/prosody次元はそれまで保守的なプレースホルダ値とする。

OSS Worker（Haskell workerおよびpython-analyzer）はCPU動作をMustとする。GPU利用は任意最適化とし、GPU backendやアクセラレーター固有情報をHTTP契約、ACL、UseCaseへ漏らさない。python-analyzerはオニオン構成とし、内側→外側のimport方向をast-grep（Python）とruffで強制する（[ADR-005](../../adr/005-python-analysis-service-onion-architecture.md)）。

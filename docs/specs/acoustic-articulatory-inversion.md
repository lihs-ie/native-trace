...
<!-- 設計の正 / 背景:
       adr/019-acoustic-to-articulatory-inversion-enrichment-service.md (Proposed, 2026-06-18)
         ...
         D6 (2026-06-19 追補): AAI-conditioned 天井偏差 step + targetArticulation 契約化 + chrome。
             frontend presentation のみ。worker/aai/wire 変更なし。
...
### D6 — AAI-conditioned 天井偏差 step + targetArticulation 契約化 + chrome (2026-06-19 追補)

<!-- D6 scope: frontend presentation のみ。worker/aai/wire/scoreImpact に触れない。
     設計権威: adr/019 D6 section (lines 124-137, 152-155, 201-211, 165) +
               applications/frontend/design-reference/screens/articulation-card.html:79,97-107。
     実装状態: ArticulationCard.tsx の DISPLAY_ELIGIBILITY_THRESHOLD(55行), normalizedCoordToPercent(51行),
               showEmaOverlay ゲート(216-218行), .ema-target(253-264行), <ol artic-steps>(377-381行),
               .disclaimer(387-394行), .artic-audio(404行以降) は D1-D5 で既 landed。
               targetArticulation は articulation-data.ts(35-42行)に存在済み。
               D6 はこれらに天井 step/バッジ/disclaimer 句/ミニマルペアボタンを乗せる追補。 -->

- [ ] **M-AAI-19 (天井偏差 step が `.artic-steps` 末尾に追記される — D6)**
  `ArticulationCard.tsx` において、`articulatoryEstimate != null` かつ
  `articulatoryEstimate.displayEligibility >= DISPLAY_ELIGIBILITY_THRESHOLD` (= 0.55) かつ
  `entry.targetArticulation` が設定済みのとき、`<ol className="artic-steps">` の末尾に
  天井 step として `<li>` を 1 件追加すること。
  文言体裁は design HTML:99 に従い:
  `天井: 破線 = 目標、塗り = あなたの推定舌先。舌先が目標より{方向}しています。`
  `{方向}` は以下の**決定論的ルール**で導出すること (worker 非依存、presentation-only):

  **偏差方向の導出アルゴリズム (ε = 2.0 パーセントポイント)**
  - `estTipXpercent = (articulatoryEstimate.tongueTipX * 0.5 + 0.5) * 100`
    (D1-D5 の `normalizedCoordToPercent` と同じ写像)
  - `estTipYpercent = (-articulatoryEstimate.tongueTipY * 0.5 + 0.5) * 100`
    (Y は **符号反転** — `normalizedCoordToPercent(-tongueTipY)` のレンダリングと一致)
  - `deltaX = estTipXpercent - entry.targetArticulation.x`
    - `deltaX > ε` → 方向ラベル X = `「後退」` (x 軸は 前→後)
    - `deltaX < -ε` → 方向ラベル X = `「前進」`
    - `|deltaX| <= ε` → X ラベルなし
  - `deltaY = estTipYpercent - entry.targetArticulation.y`
    - `deltaY > ε` → 方向ラベル Y = `「下降」` (y 軸は 上→下、スクリーン下方向)
    - `deltaY < -ε` → 方向ラベル Y = `「上昇」`
    - `|deltaY| <= ε` → Y ラベルなし
  - X ラベルと Y ラベルを両方持つ場合は `「・」` で結合 (例: `「後退・下降」`)。
  - 片方のみの場合はそのまま単独 (例: `「後退」`)。
  - 両方ともラベルなし (`|deltaX| <= ε` かつ `|deltaY| <= ε`) の場合は、
    偏差テンプレートに `{方向}` を機械的に差し込むと
    「舌先が目標よりほぼ目標どおりしています。」という非文になるため、
    偏差文そのものを `「舌先はほぼ目標どおりです。」` に差し替えること
    (文法的正しさを優先。design HTML:99 はずれ表示のみを描いており within-ε 文言を pin していない。
     受入は substring `「ほぼ目標どおりです」` で確認する)。
  - ε = 2.0 はコンポーネントローカル定数 `DEVIATION_EPSILON_PERCENT = 2.0` として定義し、
    テストで明示的に参照できるようにすること。

- [ ] **M-AAI-20 (天井 step が degrade 時に出ない — D6)**
  以下のいずれか 1 つでも成立するとき、天井 step を出さず `entry.steps` のみを描くこと
  (D2/D4 degrade と整合):
  - `articulatoryEstimate == null`
  - `articulatoryEstimate.displayEligibility < DISPLAY_ELIGIBILITY_THRESHOLD` (< 0.55)
  - `entry.targetArticulation` が未設定 (`undefined`)

- [ ] **M-AAI-21a (ADR ステータスバッジ — D6 chrome)**
  AAI カード見出し行 (設計 HTML:79) に
  `<span className="adr-badge adr-badge--proposed">ADR-019 · Proposed</span>` を置くこと。
  `adr-badge--proposed` クラスは design-components.css に既存であり、新クラスを作らないこと。

- [ ] **M-AAI-21b (disclaimer 句の補完 — D6 chrome)**
  `showEmaOverlay` が true のとき (= AAI enrichment 表示中)、`.disclaimer` 内の文言に
  design HTML:101 の落ちている句を追記すること:
  `「生 mm・舌体・下顎は出さず、発話内 z 正規化座標のみ。aai 無効時は床のみ。」`
  既存の「あなたの舌位置を断定するものではありません。」の文は**残す**こと (削除禁止)。
  追記句はその文の直後または改行なしに連結して配置すること。

- [ ] **M-AAI-21c (ミニマルペアボタン — D6 chrome)**
  `applications/frontend/src/lib/articulation-data.ts` の `ArticulationEntry` 型に
  `minimalPair?: { targetWord: string; contrastWord: string; contrastIpaDisplay: string }` を追加すること。
  このフィールドは `japanese-l1-catalog.json` confusionSet 由来の static floor データとして
  `ARTICULATION_DATA` の各エントリに直接記述すること
  (training infrastructure `drill-content.ts` を ArticulationCard から import しないこと — 層違反)。
  11 音素のうち以下の 6 音素にペアを設定し、残り 5 音素 (/ɪ/ / /ʌ/ / /ð/ / /f/ / /ə/) は
  フィールドを省略すること (ボタン非表示):

  | 音素 | targetWord | contrastWord | contrastIpaDisplay |
  |------|-----------|-------------|-------------------|
  | /l/  | light     | right       | /r/               |
  | /r/  | right     | light       | /l/               |
  | /v/  | van       | ban         | /b/               |
  ...
  `ArticulationCard.tsx` の `.artic-audio` に、`entry.minimalPair` が存在するとき
  reference TTS お手本ボタンと並べて以下のボタンを追加すること (design HTML:105 文言):
  `▸ {entry.minimalPair.targetWord} · ミニマルペア`
  ボタンのクリックで `entry.minimalPair.targetWord` を TTS 再生し、完了後に
  `entry.minimalPair.contrastWord` を続けて TTS 再生すること (既存 `/api/v1/tts` ルートを使用、
  新規スコアリング経路は一切追加しないこと)。

- [ ] **M-AAI-22 (scoreImpact 不変アサート — D6 coverage extension of M-AAI-17)**
  M-AAI-17 の unit test カバレッジを D6 経路に拡張すること。具体的には:
  (a) backend: M-AAI-17 の既存テストが `cabal test all` で引き続き緑であること
      (D6 は backend を変更しないため、`grep -n "substitution\|omission\|insertion\|epenthesis"
      applications/backend/src/NativeTrace/Worker/Scoring.hs` の allow-list が不変であること)。
  (b) frontend: `ArticulationCard` の天井 step 導出が `articulatoryEstimate` と
      `entry.targetArticulation` のみを参照し、`finding.scoreImpact` / severity / ScoreSet の
      いずれのフィールドも読まないことを component test で assert すること。
      具体的には `scoreImpact` を含む props を与えたカードと含まない props のカードで
      天井 step の文言が一致することを確認する (presentation-only の機械検証)。

- [ ] **M-AAI-23 (runtime — live worker→aai→frontend 観測 assert — D6 / high-risk)**
  AAI 有効・高適格性 vowel/approximant の実録音で live `worker→aai→frontend` を通し、
  `ArticulationCard` に以下が同時に出ることを観測 assert すること:
  - EMA オーバーレイ (`.ema-layer` / `.ema-pt--tip` 等)
  - 破線目標丸 (`.ema-target`、`entry.targetArticulation` 座標)
  - 天井偏差 step (design HTML:99 体裁の文言を含む `<li>`)
  - ADR ステータスバッジ (`adr-badge--proposed` を含む `<span>`)
  - 補完済 disclaimer (「生 mm・舌体・下顎は出さず…」句を含む `.disclaimer`)
  - ミニマルペアボタン (対象音素が /l//r//v//θ//æ//iː/ のとき `.artic-audio` 内に存在)

  かつ低適格性 (displayEligibility < 0.55) / articulatoryEstimate=null / aai 停止の各ケースで
  天井 step が描画されず floor (`entry.steps` のみ) に degrade することを観測 assert すること。

  **aai live 起動コマンド** (memory: docker-rebuild-required-for-code-changes):
  `docker compose --profile aai up -d --build aai`

  **代替 (live aai inference が in-session で実行不能な場合):**
  - component-level render assert (M-AAI-19〜21 の unit test) を緑にすること。
  - worker contract roundtrip (articulatoryEstimate が wire を通って ArticulationCard props に届く
    end-to-end TS test) を緑にすること。
  - `commands.txt` に「live aai inference 未実施のため component-level 代替とした理由」と
    「live 検証を実施するコマンド・前提条件 (docker compose --profile aai up --build aai + 実録音投入)」
    を明記し、runtime-pending フラグを立てること (ADR-024 の live smoke 代替前例と同型)。

  証跡は `.agent-evidence/acoustic-articulatory-inversion/d6/` 以下の
  `commands.txt` / `wiring-map.json` / `completion-report.md` に記録すること。
...
### D6 acceptance (M-AAI-19〜23 の確認方法)

- **M-AAI-19** →
  `grep -n "天井\|DEVIATION_EPSILON_PERCENT\|後退\|前進\|下降\|上昇\|ほぼ目標どおり" applications/frontend/src/components/workspace/ArticulationCard.tsx`
  で天井 step 文言と epsilon 定数の存在を確認すること。
  `grep -n "DEVIATION_EPSILON_PERCENT" applications/frontend/src/components/workspace/ArticulationCard.tsx`
  が `DEVIATION_EPSILON_PERCENT = 2.0` を含むこと。
  component test (ε ピン留め): 以下の 4 ケースを unit test で assert すること。
  - `tongueTipX=0.6` (estTipXpercent=80%) / `tongueTipY=-0.5` (estTipYpercent=75%) /
    `targetArticulation.x=55` / `targetArticulation.y=41` → deltaX=25 > ε, deltaY=34 > ε → 「後退・下降」
  - `tongueTipX=-0.2` (estTipXpercent=40%) / `tongueTipY=0.3` (estTipYpercent=35%) /
    `targetArticulation.x=55` / `targetArticulation.y=41` → deltaX=-15 < -ε, deltaY=-6 < -ε → 「前進・上昇」
  - `tongueTipX=0.1` / `tongueTipY=-0.3` / `targetArticulation.x=50` / `targetArticulation.y=35` →
    estTipXpercent=55, estTipYpercent=65, deltaX=5 > ε, deltaY=30 > ε → 「後退・下降」 (片軸のみでもよい例)
  - `articulatoryEstimate.tongueTipX` が `targetArticulation.x` に ε 以内: 当該軸のラベル省略。
  上記 assert で `entry.steps` とは**別個の** `<li>` が末尾に追加されることを DOM で確認すること。
  `pnpm test --run` 緑。

- **M-AAI-20** →
  component test: `articulatoryEstimate=null` / `displayEligibility=0.4` (< 0.55) /
  `entry.targetArticulation=undefined` の 3 ケース各々で天井 step が DOM に存在しないことを assert すること
  (`querySelectorAll("ol.artic-steps li").length` が `entry.steps.length` と一致)。
  `pnpm test --run` 緑。

- **M-AAI-21a** →
  `grep -n "adr-badge--proposed\|ADR-019.*Proposed" applications/frontend/src/components/workspace/ArticulationCard.tsx`
  で badge span が確認できること。
  component test: render した DOM に `adr-badge--proposed` クラスを持つ要素が存在し
  テキストに「ADR-019」と「Proposed」を含むことを assert すること。
  `grep -rn "adr-badge--proposed" applications/frontend/src/styles/` で**既存**クラス定義が確認できること
  (新規 CSS 追加なし)。
  `pnpm test --run` / `pnpm typecheck` / `pnpm lint` 緑。

- **M-AAI-21b** →
  `grep -n "生 mm\|舌体・下顎\|aai 無効時は床のみ" applications/frontend/src/components/workspace/ArticulationCard.tsx`
  で追記句が確認できること。
  component test: `showEmaOverlay=true` 相当の props で render し、`.disclaimer` のテキストが
  「生 mm・舌体・下顎は出さず、発話内 z 正規化座標のみ。aai 無効時は床のみ。」を含むことを assert すること。
  既存文「あなたの舌位置を断定するものではありません。」が同じ `.disclaimer` 内に残ることも assert すること。
  `pnpm test --run` 緑。

- **M-AAI-21c** →
  `grep -n "minimalPair" applications/frontend/src/lib/articulation-data.ts`
  で `ArticulationEntry.minimalPair` 型定義と 6 音素への設定値が確認できること。
  `grep -n "light.*right\|right.*light\|van.*ban\|think.*sink\|cat.*cut\|seat.*sit" applications/frontend/src/lib/articulation-data.ts`
  で 6 ペアの word データが確認できること。
  `grep -n "ミニマルペア\|minimalPair\|contrastWord" applications/frontend/src/components/workspace/ArticulationCard.tsx`
  でボタン実装と sequential TTS 再生が確認できること。
  `grep -rn "drill-content" applications/frontend/src/components/workspace/ArticulationCard.tsx` が 0 件
  (training infrastructure import なし)。
  component test: `/l/` 音素の props で render し `.artic-audio` 内に「light · ミニマルペア」ボタンが
  存在することを assert すること。
  component test: `/ə/` (minimalPair フィールド未設定) の props で render し `.artic-audio` 内に
  「ミニマルペア」ボタンが存在しないことを assert すること。
  `pnpm test --run` / `pnpm typecheck` / `pnpm fitness` 緑。

- **M-AAI-22** →
  (a) `cabal test all` 緑 (D6 は backend を変更しないため M-AAI-17 テストが引き続き通過)。
  `grep -n "substitution\|omission\|insertion\|epenthesis" applications/backend/src/NativeTrace/Worker/Scoring.hs`
  で allow-list が不変であること。
  (b) component test: `ArticulationCard` を `scoreImpact: "major"` を含む finding props で render した場合と
  `scoreImpact: "minor"` で render した場合で、天井 step の文言が props の scoreImpact 値と**無関係**であることを
  assert すること (同一 `articulatoryEstimate` + `targetArticulation` に対して同じ方向ラベルが出る)。
  `grep -n "scoreImpact" applications/frontend/src/components/workspace/ArticulationCard.tsx` で
  天井 step 導出ロジック内に `scoreImpact` が現れないことを確認すること。
  `pnpm test --run` 緑。

- **M-AAI-23** →
  live 実施時:
  `docker compose --profile aai up -d --build aai` で aai 起動後、実録音 (vowel/approximant) を
  `/api/v1/pronunciation-assessments` に投入し、UI の ArticulationCard に M-AAI-23 の 6 要素
  (EMA overlay / .ema-target / 天井 step / adr-badge--proposed / disclaimer 追記句 / ミニマルペアボタン)
  が同時に出ることをブラウザ DOM inspector または Playwright で観測 assert し、実値を
  `.agent-evidence/acoustic-articulatory-inversion/d6/commands.txt` に記録すること。
  低適格性・null・aai 停止ケースで天井 step と EMA overlay が消えることも観測すること。

  component-level 代替 (live 未実施時):
  `grep -n "天井\|adr-badge--proposed\|ミニマルペア\|生 mm" applications/frontend/src/components/workspace/ArticulationCard.tsx`
  で D6 chrome の全要素が 1 コマンドで確認できること。
  M-AAI-19〜21 の unit test が全て緑であること。
  `commands.txt` に runtime-pending フラグと live 検証手順を明記すること。
  `.agent-evidence/acoustic-articulatory-inversion/d6/` の 3 ファイルが存在すること。
  `pnpm lint` / `pnpm typecheck` / `pnpm test --run` / `pnpm fitness` 緑。
...
- **D6: worker/aai/wire の変更**: D6 は frontend presentation のみ。worker/aai service/compose.yaml/
  wiring_manifest.yml を変更しないこと。
- **D6: 新 CSS クラスの追加**: 天井 step / バッジ / disclaimer 句 / ミニマルペアボタンは
  design-components.css に既存のクラス (`adr-badge--proposed` / `.disclaimer` / `.artic-audio` /
  `.artic-steps`) のみを使う。新クラスを定義しないこと。
- **D6: 天井偏差量の score/severity 変換**: 偏差方向ラベルは表示テキスト生成のみで、
  スコア/重大度/ScoreSet に一切変換しないこと (presentation-only)。
- **D6: re-record EMA delta**: 再録音後「EMA が目標調音へ動いたか」を計測する delta 表示は
  S-AAI-4 の将来拡張点であり、本 D6 の scope 外。
- **D6: worker 側での偏差方向導出**: `targetArticulation` は frontend floor データ
  (articulation-data.ts) であり worker に存在しない。worker に偏差フィールドを追加すると
  D3-c の wire 最小化と矛盾するため採用しない (ADR-019 alt line 165)。
...
- 理由 (触れる境界領域):
  ...
  - **D6 固有: frontend presentation-only だが M-AAI-23 live runtime は worker→aai→frontend 境界を越える**:
    D6 のコード変更自体は frontend 内のみ (medium risk) だが、M-AAI-23 の runtime 検証は D1-D5 の
    worker→aai→frontend 全経路を通す。この live verify フェーズは cross-language wire 契約境界を踏むため、
    親 ADR-019 と同じ `high-risk` / `escalate_to_opus: true` を維持する。
    spec-grade / done-evaluator (二段門) も Opus 4 での検証が必要。
...
- v3 デザインは既に live CSS に逐語移植済みで、これがオーバーレイ合成の権威:
  ...
  - `design-components.css` (既存): `.adr-badge` / `.adr-badge--proposed` (D6 バッジ)。
...
- D6 追加要素のデザイン権威 (`applications/frontend/design-reference/screens/articulation-card.html`):
  - line 79: `<span class="adr-badge adr-badge--proposed">ADR-019 · Proposed</span>` — バッジ
  - lines 97–99: `<ol class="artic-steps">` 内の天井 step — 「天井: 破線 = 目標、塗り = あなたの推定舌先。
    舌先が目標より後退・下降しています。」
  - line 101: `.disclaimer` 追記句 — 「生 mm・舌体・下顎は出さず、発話内 z 正規化座標のみ。aai 無効時は床のみ。」
  - lines 103–107: `.artic-audio` — reference TTS ボタン + ミニマルペアボタン + 録音ボタン + Kocjancic 注記。
...
**D6 追補 (2026-06-19):**
なし。以下の判断は全て確定済み:
- ミニマルペア再生の動作: targetWord → contrastWord を sequential TTS 再生 (既存 /api/v1/tts)。
- 偏差方向の導出: frontend presentation で `articulatoryEstimate` × `targetArticulation` から決定論計算
  (ε = 2.0 パーセントポイント); worker 側導出は不採用 (ADR-019 alt line 165)。
- ミニマルペア適用音素: /l/ /r/ /v/ /θ/ /æ/ /iː/ の 6 音素 (drill-content.ts confusionSet 由来)。
  /ɪ/ /ʌ/ /ð/ /f/ /ə/ はフィールド省略でボタン非表示。
- targetArticulation の doc comment: 「ADR-019 D6 正式契約フィールド」を追記し S-AAI-5(b) 校正注記を維持。
